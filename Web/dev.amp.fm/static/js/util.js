...
$.get("/index.page", function(data){
var ctx = new ExprContext(data);
var normalResults = expr.evaluate(ctx);
$.log($(normalResults.nodeSetValue()).text());
});*/

//$.getScript("http://dev.virtualearth.net/mapcontrol/mapcontrol.ashx");

//$.log(match);
//matching = new Matching('friday', 'frdiay', 2).areNeighbors();
...
function outputVars(path) {
    ...
    for (var i = 0; i < viewLength; i++) {
        ...
        switch (current[0]) {
            case 'perspective':
                outputArray["perspective"] = (current[1]) ? current[1] : 'home';
                break;
            case 'timespan':
                outputArray["timespan"] = (current[1]) ? current[1] : 'today';
                break;
            case 'view':
                outputArray["view"] = (current[1]) ? current[1] : 'newspaper';
                break;
            case 'location':
                outputArray["location"] = (current[1]) ? current[1] : M_LOCATION;
                break;
            case 'filters':
                outputArray["filters"] = (current[1]) ? current[1] : '';
                break;
            default:
                break;
        }
...
function writeDay() {
    $('#cal-day').text(date.getDate().toString());
}
function writeMonth() {
    $('#cal-month').text(monthArray[date.getMonth()].toString());
}
