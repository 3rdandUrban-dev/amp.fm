<xsl:transform version="2.0"
  ...
  <xsl:function name="func:format-dateTime-string">
    ...
    <xsl:variable name="formattedDateTime" select="if(contains($dateTime, 'T')) then format-dateTime(xs:dateTime(if(string-length(substring-before(substring-after($dateTime, 'T'), 'Z')) lt 8) then concat(substring-before($dateTime, 'Z'), ':00Z') else $dateTime), $picture) else if(empty($dateTime)) then '' else ''"/>
    <xsl:if test="not(empty($dateTime))">
      <xsl:value-of select="$formattedDateTime"/>
    </xsl:if>
  </xsl:function>
  
  <xsl:function name="func:test-date-format">
  <xsl:param name="dateTime"/>
  <xsl:variable name="formattedDateTime" select="format-dateTime(xs:dateTime(if(string-length(substring-before(substring-after($dateTime, 'T'), 'Z')) lt 8) then concat(substring-before($dateTime, 'Z'), ':00Z') else $dateTime), '[MNn] [Do] [Y] [H01]:[m01]:[s01] [z]')"/>
  <xsl:if test="not(empty($dateTime))">
      <xsl:value-of select="$formattedDateTime"/>
    </xsl:if>
  </xsl:function>
