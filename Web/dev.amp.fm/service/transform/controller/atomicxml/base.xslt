<xsl:transform xmlns="http://www.w3.org/1999/xhtml" xmlns:html="http://www.w3.org/1999/xhtml"
  ...
  exclude-result-prefixes="omx app atom html" version="2.0">
  ...
  <xsl:template match="omx:list[@src and not(@select)]">
    <ul id="{if (@id) then (@id) else generate-id()}">
      ...
      <xsl:apply-templates
        select="document(resolve-uri(@src, $xml.base))/app:service/app:workspace/atom:title"/>
    </ul>
  </xsl:template>
  ...
  <xsl:template match="omx:list[@src and @select]">
    <ul id="{if (@id) then (@id) else generate-id()}">
      ...
      <xsl:apply-templates
        select="document(resolve-uri(@src, $xml.base))/app:service/app:workspace/app:collection"/>
    </ul>
  </xsl:template>
