...
-->
<xsl:stylesheet xmlns:html="http://www.w3.org/1999/xhtml" xmlns:at="http://atomictalk.org/2009/AtomicTalk#" xmlns:pledge="http://4lessig.org/pledge" xmlns:debug="http://nuxleus.com/session/debug" xmlns:request-collection="clitype:Xameleon.Function.HttpRequestCollection?partialname=Xameleon" xmlns:request="http://atomictalk.org/function/aspnet/request" xmlns:session="http://atomictalk.org/session" xmlns:geo="http://nuxleus.com/geo" xmlns:my="http://xameleon.org/my" xmlns:page="http://atomictalk.org/page" xmlns:doc="http://atomictalk.org/feed/doc" xmlns:service="http://atomictalk.org/page/service" xmlns:output="http://atomictalk.org/page/output" xmlns:head="http://atomictalk.org/page/output/head" xmlns:body="http://atomictalk.org/page/output/body" xmlns:advice="http://atomictalk.org/page/advice" xmlns:view="http://atomictalk.org/page/view" xmlns:layout="http://atomictalk.org/page/view/layout" xmlns:form="http://atomictalk.org/page/view/form" xmlns:menu="http://atomictalk.org/page/view/menu" xmlns:exsl="http://exslt.org/common" xmlns:resource="http://atomictalk.org/page/resource" xmlns:model="http://atomictalk.org/page/model" xmlns:app="http://www.w3.org/2007/app" xmlns:atompub="http://www.w3.org/2007/app" xmlns:aspnet-context="clitype:System.Web.HttpContext?partialname=System.Web" xmlns:response="http://atomictalk.org/function/aspnet/response" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:func="http://atomictalk.org/function" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:saxon="http://saxon.sf.net/" xmlns:fb="http://www.facebook.com/2008/fbml" version="2.0">
  ...
  <xsl:variable name="virtual-path" select="substring-before(request:get-raw-url(), '/index.page')"/>
  ...
  <xsl:variable name="sourcedoc" select="/service:compile/service:session"/>
  <xsl:variable name="servicedoc" select="document(resolve-uri(concat('file://', $application-root, /service:compile/service:session/service:doc/@src)))"/>
  <xsl:variable name="page" select="$sourcedoc/my:page"/>
  ...
  <xsl:variable name="page.output.head" select="document(resolve-uri(concat('file://', $application-root, $page/page:output/page:head/@src)))/page:head/*|$page/page:output/page:head/*"/>
  ...
  <xsl:variable name="page.output.body">
    <xsl:choose>
      <xsl:when test="$page/page:output/page:body/@src">
        <xsl:apply-templates select="document(resolve-uri(concat('file://', $application-root,$page/page:output/page:body/@src)))/page:body" />
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates select="$page/page:output/page:body"/>
      </xsl:otherwise>
  ...
  <xsl:template match="geo:*|embed">
    <xsl:copy-of select="."/>
  </xsl:template>

  <xsl:template match="at:*"/>

  <xsl:template match="menu:navigation">
    <xsl:variable name="source-namespace" select="substring-before(@match, ':')"/>
    <xsl:variable name="source">
      <xsl:choose>
        <xsl:when test="$source-namespace = '$sourcedoc'">
          <xsl:text>$p1</xsl:text>
        </xsl:when>
        <xsl:when test="$source-namespace = '$servicedoc'">
    ...
    <xsl:variable name="selected">
      <xsl:call-template name="replace">
        <xsl:with-param name="string" select="@selected"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="match" select="saxon:evaluate($source, $sourcedoc, $servicedoc, .)"/>
    <xsl:variable name="expression">
      <xsl:call-template name="replace">
        <xsl:with-param name="string" select="concat('/', substring-after(@match, ':'))"/>
      </xsl:call-template>
    </xsl:variable>
    <ul class="menu {@type}" id="{@id}-menu">
      <xsl:apply-templates select="$match" mode="evaluate-workspace-expression">
  ...
  <xsl:template match="*" mode="evaluate-workspace-expression">
    <xsl:param name="expression"/>
    <xsl:param name="use"/>
    <xsl:param name="selected"/>
    <xsl:apply-templates select="saxon:evaluate($expression)" mode="evaluate-use-expression">
      <xsl:with-param name="use" select="$use"/>
      <xsl:with-param name="selected" select="$selected"/>
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="*" mode="evaluate-use-expression">
    <xsl:param name="use"/>
    <xsl:param name="selected"/>
    <xsl:apply-templates select="saxon:evaluate($use)" mode="workspace-menu">
      <xsl:with-param name="selected" select="$selected"/>
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="app:workspace" mode="workspace-menu">
    <xsl:param name="selected"/>
    <xsl:variable name="selected-title" select="saxon:evaluate($selected)"/>
    <xsl:apply-templates select="app:collection" mode="workspace-menu">
      <xsl:with-param name="selected" select="$selected-title"/>
    </xsl:apply-templates>
  </xsl:template>

  <xsl:template match="app:collection" mode="workspace-menu">
    <xsl:param name="selected"/>
    <xsl:variable name="title" select="atom:title"/>
    <xsl:variable name="translated-title" select="lower-case(translate($title, ' ', '-'))"/>
    <li id="{$translated-title}-selector" class="{if ($selected/atom:title = $title) then 'selected' else 'not-selected'}">
      <a rel="workspace" href="{@href}">
        <xsl:value-of select="$title"/>
      </a>
    </li>
  </xsl:template>

  <xsl:template match="atom:category" mode="workspace-menu">
    <xsl:param name="selected"/>
    <xsl:variable name="title" select="@label"/>
    <xsl:variable name="uri" select="concat(@scheme, @term)"/>
    <xsl:variable name="translated-title" select="lower-case(translate($title, ' ', '-'))"/>
    <li id="{$translated-title}-selector" class="{if ($selected = $title) then 'selected' else 'not-selected'}">
      <a rel="workspace" href="{$uri}">
        <xsl:value-of select="$title"/>
      </a>
    </li>
  </xsl:template>
  ...
  <xsl:template match="page:config">
    <page:config>
      <page:advice>
        <xsl:apply-templates select="document(resolve-uri(concat('file://', $application-root,$page/page:config/@src)))/page:config/page:advice/*" mode="resolve"/>
  ...
  <xsl:template match="view:module[@src]|view:item[@src]">
    <xsl:apply-templates select="document(resolve-uri(concat('file://', $application-root,@src)))"/>
  </xsl:template>
  ...
  <xsl:template match="service:doc"/>
